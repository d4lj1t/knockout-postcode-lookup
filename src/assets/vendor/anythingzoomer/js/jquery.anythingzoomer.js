...
;(function($){
	...
	$.anythingZoomer = function(el, options){
		...
		base.init = function(){
			...
			base.$inner
				.bind('mouseenter' + n.namespace + ' touchstart' + n.namespace, base.onmouseenter)
				.bind('mouseleave' + n.namespace + ' touchend' + n.namespace, base.onmouseleave)
				.bind('mousemove' + n.namespace + ' touchmove' + n.namespace, base.onmousemove)
		...
		base.onmouseenter = function(e) {
			...
			if (o.delay) {
				...
				base.reveal();
				// Show the correct part of the image when the user touches the canvas
		...
		base.onmousemove = function(e) {
			...
			// Is it a mousemove or touchmove
			...
			if( e.type === "touchmove" ) {
				...
				var $parent = $(element).parents("." + n.wrap).eq(0);
				var $parentInner = $parent.find("." + n.wrapInner);
				var $item = $(element).parents(".item").eq(0);

				if( $(element).parents("." + n.wrap).eq(0).is( base.$wrap ) ) {
					console.log('This is the same element');
				} else {
			...
			if (base.state && base.enabled){
				...
				base.zoomAt( pageX - off.left, pageY - off.top, null, true );
			}
